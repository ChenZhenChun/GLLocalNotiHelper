Pod::Spec.new do |s|
#name必须与文件名一致
s.name              = "GLLocalNotiHelper"

#更新代码必须修改版本号
s.version           = "0.0.2"
s.summary           = "Manage local notifications for ios."
s.description       = <<-DESC
It is a GLLocalNotiHelper used on iOS, which implement by Objective-C.
DESC
s.homepage          = "https://github.com/ChenZhenChun/GLLocalNotiHelper"
s.license           = 'MIT'
s.author            = { "ChenZhenChun" => "346891964@qq.com" }

#submodules 是否支持子模块
s.source            = { :git => "https://github.com/ChenZhenChun/GLLocalNotiHelper.git", :tag => s.version, :submodules => true}
s.platform          = :ios, '8.0'
s.requires_arc = true

#source_files路径是相对podspec文件的路径

#核心模块
s.subspec 'GLLocalNotiHelper' do |ss|
ss.source_files = 'GLLocalNotiHelper/GLLocalNotiHelper/*.{h,m}'
ss.public_header_files = 'GLLocalNotiHelper/GLLocalNotiHelper/*.h'
end

#子模块image(存放声音)
#s.subspec 'source' do |ss|
#ss.resources = 'GLLocalNotiHelper/source/*.caf'
#end

s.frameworks = 'Foundation', 'UIKit'
#引用了第三方库
s.dependency "MJExtension"

# s.ios.exclude_files = 'Classes/osx'
# s.osx.exclude_files = 'Classes/ios'
# s.public_header_files = 'Classes/**/*.h'

end
